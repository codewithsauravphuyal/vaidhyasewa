...
interface HeroProps {
  ...
  title: string;
  title_2: string;
  description: string | string[];  // Supports string or array
}

export function Herosection({ title, title_2, description, subtitle }: HeroProps) {
  // Normalize to array: split string by \n, or use array as-is
  ...
  style={{
    ...
    backgroundPosition: 'top center', // mobile top
    ...
    <div className="relative z-10 max-w-4xl pt-20 md:pt-0">
      <div className="text-left">
        ...
        <h1 className="text-3xl md:text-4xl lg:text-[36px] font-semibold text-bluegray_900 mb-4">
          {title}{' '}
          <span className="text-teal_700">{title_2}</span>
        </h1>
